...
export class RoleListComponent extends BaseCmsListComponent implements OnInit, OnChanges {
  ...
  override responseList: JfResponseList<Role> = new JfResponseList<Role>(0, 0, [])
  ...
  constructor(
    ...
    super()
    this.queryFieldOptions = [this.labels.role.name, this.labels.role.description]
  ...
  override onLazyLoad(strAction = ''): void {
    ...
    this.modelSearch.lazyLoadEvent.select = [`${this.labels.role.id.field}`, `${this.labels.role.name.field}`]
    ...
    switch (strAction) {
      case 'export':
        const csvColumns: any = JfUtils.csvColumns(this.labels.role)
        this.modelSearch.lazyLoadEvent.additional.push(new JfCondition('action', strAction))
        this.modelSearch.lazyLoadEvent.additional.push(new JfCondition('title', this.labels.role.ownNamePlural))
        ...
        this.crudService.export(kRoute, this.modelSearch.lazyLoadEvent).subscribe(
          (resp) => {
            this.loading = false
            JfUtils.downloadFile(resp, this.labels.role.ownNamePlural)
          },
          (error) => {
            this.loading = false
